...
void main(){
    ...
    float start_serial = omp_get_wtime();
    mergeSortSerial(arr, 0, n-1);
    float end_serial = omp_get_wtime();
    float time1 = end_serial - start_serial;

    float start_parallel = omp_get_wtime();
    mergeSortParallel(arr2, 0, n-1);
    float end_parallel = omp_get_wtime();
    float time2 = end_parallel - start_parallel;
